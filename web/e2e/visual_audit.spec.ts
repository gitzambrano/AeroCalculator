...
for (const viewport of cases) {
  test(`visual audit ${viewport.name}`, async ({ browser }) => {
    ...
    await shot(page, dir, "08-main-menu");
    ...
    await shot(page, dir, "13-editor-collapsed");
    await page.locator("#add-flap").click();
    await shot(page, dir, "14-editor-one-flap");
    ...
    await shot(page, dir, "17-vs-factor-delta");
    ...
    await shot(page, dir, "18-wind-speed-direction");

    if (viewport.name === "mobile-320" || viewport.name === "mobile-390") {
      await page.getByRole("button", { name: "INPUTS" }).click();
