...
def b(s):
 m=re.fullmatch(r'\[(-?\d+),(-?\d+)\]\[(-?\d+),(-?\d+)\]',s or ''); return tuple(map(int,m.groups())) if m else None
def ctr(n):
 x1,y1,x2,y2=b(n.attrib['bounds']); return (x1+x2)//2,(y1+y2)//2
...
def find(root,t):
 q=norm(t)
 for n in nodes(root):
  if norm(n.attrib.get('text',''))==q:return n
 return None
def parents(root): return {c:p for p in root.iter() for c in p}
def tap(n):
 x,y=ctr(n); adb('shell','input','tap',str(x),str(y)); time.sleep(.7)
def taptext(t):
...
def scroll(root,up=True):
 sv=next((n for n in nodes(root) if n.attrib.get('class')=='android.widget.ScrollView' and n.attrib.get('scrollable')=='true'),None); rec('scrollview',sv is not None,'missing')
 x1,y1,x2,y2=b(sv.attrib['bounds']); x=(x1+x2)//2; h=y2-y1; sy=y1+int(h*(.82 if up else .28)); ey=y1+int(h*(.28 if up else .82))
 adb('shell','input','swipe',str(x),str(sy),str(x),str(ey),'420'); time.sleep(.6)
...
def layout(root,name):
 ns=nodes(root); rec('app-tree:'+name,bool(ns),'empty'); rb=b(ns[0].attrib.get('bounds','')); rec('root-bounds:'+name,rb is not None,str(rb)); x1,_,x2,_=rb; bad=[]
 for n in ns:
  z=b(n.attrib.get('bounds',''))
...
def icon(which):
 r=dump('icon-'+which); rb=b(nodes(r)[0].attrib['bounds']); w=rb[2]-rb[0]; d=440/160; panel=w*.03+32*d; x=int(rb[2]-(1.5 if which=='plus' else .5)*panel); y=int(rb[1]+28*d)
...
def main():
 ...
 adb('shell','monkey','-p',PKG,'-c','android.intent.category.LAUNCHER','1'); time.sleep(3); fg('launch')
 ...
 top(); taptext('CALCULATE'); time.sleep(1); fg('calculate'); r=dump('outputs-top'); layout(r,'outputs-top'); shot('outputs-top')
 ...
 taptext('INPUTS'); taptext('AIRPLANES'); fg('airplanes'); r=dump('airplanes'); layout(r,'airplanes'); shot('airplanes')
 icon('plus'); fg('aircraft-editor'); r=dump('aircraft-editor'); rec('editor-open',all(find(r,x) is not None for x in ['Save','Cancel','Name']),str(texts(r)[:80])); layout(r,'editor'); shot('editor-top'); before=tuple(texts(r))
 for _ in range(5):r=dump('editor-scroll'); scroll(r,True)
 r=dump('editor-bottom'); rec('editor-scroll',tuple(texts(r))!=before,str(texts(r)[:80])); shot('editor-bottom'); c=find(r,'Cancel'); tap(c) if c is not None else adb('shell','input','keyevent','4'); time.sleep(1); fg('editor-cancel')
 icon('menu'); r=dump('menu'); required=['Clear Inputs','Import Airplanes','Export Airplanes','Settings','Send Feedback','About']; miss=[x for x in required if find(r,x) is None]; rec('menu-items',not miss,f'missing={miss}; visible={texts(r)}'); shot('menu'); adb('shell','input','keyevent','4',check=False); fg('menu-close')
 report(); print(json.dumps(R,indent=2))

try:main()
except Exception as e:
 R['error']=f'{type(e).__name__}: {e}'; report(); shot('failure'); raise
finally:
