...
#Region Background
'Applies the new  (BackgroundColor, CheckedColor, DividerColor, ExtensionColor) to the given item
...
'		pnl = sv.Panel.GetView(i)
'		If DragAndDrop.Top + 75dip >= pnl.Top Then 
'		
'		For zz=i To i Step-1 
'		pnl = sv.Panel.GetView(i)
'		pnl.Top = (i-1)*50dip
'		Next
'		End If
...
Private Sub MoveToDropPosition(Y As Int)
	...
	For i = 0 To NumberOfItems - 3
		pnl = sv.Panel.GetView(i)
		If DragAndDrop.Top + (pnl.Height*0.4) >= pnl.Top Then 
		EndPosition = i+1
		End If
	Next
	Dim EndPosition2 As Int
	For i = NumberOfItems - 3 To 0 Step-1
		pnl = sv.Panel.GetView(i)
		If DragAndDrop.Top - (pnl.Height*0.4) <= pnl.Top Then 
			EndPosition2 = i
		End If
	Next
	If StartPosition > EndPosition2 Then
